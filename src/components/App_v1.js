...
export default function App() {
  ...
  const [prevWatched, setPrevWatched] = useLocalStorage(
    [],
    'prevWatched'
  );
  ...
  console.log(yearWatched);
  console.log(typeWatched);
  ...
  const handleApplyFiltersWatched = (newFilters) => {
    ...
    // Apply filters to movies array
    const filteredMovies = watched.filter((movieWatched) => {
      let matchesYear = true;
      let matchesType = true;
      ...
      if (newFilters.year !== '') {
        // Use newFilters here
        matchesYear =
          movieWatched.year.split('–')[0] === newFilters.year;
      }

      if (newFilters.type !== '') {
        // Use newFilters here
        matchesType = movieWatched.type === newFilters.type;
      }
  ...
  const handleRemoveFilterWatched = (filterKey) => {
    ...
    if (updatedFilters.year === '' && updatedFilters.type === '') {
      ...
      const filteredMovies = prevWatched.filter((movieWatched) => {
        let matchesYear = true;
        let matchesType = true;

        if (updatedFilters.year !== '') {
          matchesYear = movieWatched.year === updatedFilters.year;
        }
        if (updatedFilters.type !== '') {
          matchesType = movieWatched.type === updatedFilters.type;
        }
        return matchesYear && matchesType;
      });
      setWatched(filteredMovies);
  ...
  const handleSortWatchlist = (e) => {
    ...
    const sortedByProp =
      prop === 'Title'
        ? prevWatched
        : [...watched].sort((a, b) => b[prop].localeCompare(a[prop]));
    setWatched(sortedByProp);
  };
  ...
  const handleReverseWatchlist = () => {
    ...
    const reversed = [...watched].reverse();
    setWatched(reversed);
  };
  ...
  const handleAddMovie = (rating, movie) => {
    ...
    setPrevWatched((watched) => watched);
  };
          ...
          {selectedID ? (
            ...
            <>
              ...
              >
                ...
                <strong>{prevWatched.length}</strong> results
              </NumResultsWatchlist>
              <WatchedMoviesList
                watched={watched}
                onSelectMovie={handleSelectMovie}
