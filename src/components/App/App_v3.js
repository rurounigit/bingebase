...
export default function App() {
  ...
  const isLastPage =
    allPages.current.length === Number(firstPage?.data?.totalResults);
  ...
  }, [query, pages, queryClient, isLastPage, paginationDirection]);
  ...
  if (firstPage.isSuccess && nextPage.isSuccess) {
    if (
      ...
      if (allPages.current.length > pages.current * 10) {
        ...
        console.log('pages.current * 10: ', pages.current * 10);
