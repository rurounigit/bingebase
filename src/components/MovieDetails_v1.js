...
export const MovieDetails = ({
  ...
  const [summaryIsShort, setSummaryIsShort] = useState(true);
  ...
  const handleSummary = () => {
    setSummaryIsShort(!summaryIsShort);
  };
    ...
    const fetchMovieDetails = async () => {
      try {
        ...
        const res = await fetch(
          `http://www.omdbapi.com/?apikey=${KEY}&i=${selectedID}&plot='short'
          }`
        );
  ...
  }, [title, savedRating, rating, selectedID]);
